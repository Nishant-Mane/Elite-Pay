...
const navLinks = [
  ...
  { to: "/user/topup",        label: "Top Up",       icon: "💳" },
  { to: "/user/cards",        label: "My Cards",     icon: "🪪" },
...
export default function UserLayout() {
  ...
  useEffect(() => {
    const activeIdx = navLinks.findIndex(l =>
      l.to === "/user" ? location.pathname === "/user" : location.pathname.startsWith(l.to)
    );
    if (linkRefs.current[activeIdx]) {
      const el = linkRefs.current[activeIdx];
      setPillTop(el.offsetTop + 8);
    }
      ...
      <aside className={`sidebar${sidebarOpen ? " open" : ""}`}>
        <Link to="/" style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "2.25rem", textDecoration: "none" }}>
          <div style={{ width: 30, height: 30, background: "var(--cream-txt)", borderRadius: "7px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <span style={{ color: "var(--navy)", fontWeight: 900, fontSize: "0.9rem", fontFamily: "Sora, sans-serif" }}>U</span>
          </div>
        ...
        <nav style={{ flex: 1, position: "relative" }}>
          <div style={{
            position: "absolute", left: 0, top: pillTop,
            width: 3, height: 34, background: "var(--gold)", borderRadius: "0 3px 3px 0",
            transition: "top 0.35s cubic-bezier(0.34,1.56,0.64,1)", pointerEvents: "none"
          }} />
          {navLinks.map((link, i) => {
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  padding: "0.75rem 1rem", borderRadius: 10, marginBottom: "0.3rem",
                  textDecoration: "none", fontSize: "0.875rem",
                  fontWeight: active ? 600 : 400,
                  background: active ? "rgba(240,233,223,0.1)" : "transparent",
                  color: active ? "var(--cream-txt)" : "rgba(240,233,223,0.5)",
                  transition: "all 0.18s",
                  animation: bouncingIdx === i ? "bounce-icon 0.4s cubic-bezier(0.34,1.56,0.64,1)" : undefined
                }}
        ...
        <div style={{ borderTop: "1px solid rgba(196,168,130,0.18)", paddingTop: "1.25rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "1rem" }}>
            <div style={{ width: 32, height: 32, background: `${tc}22`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: tc, fontSize: "0.85rem", fontWeight: 700, flexShrink: 0 }}>
              {(user?.name || "U")[0].toUpperCase()}
            </div>
            <div style={{ overflow: "hidden" }}>
              <div style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--cream-txt)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.name}</div>
              <div style={{ fontSize: "0.68rem", color: "rgba(240,233,223,0.4)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.email}</div>
            </div>
          </div>
